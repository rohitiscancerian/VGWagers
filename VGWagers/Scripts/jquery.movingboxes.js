...
;(function($){
	...
	$.movingBoxes = function(el, options){
		...
		var delayLength = 2000;
		var sliderIntervalID;

		$('.game-img-slider').mouseover(function () {
		    clearInterval(sliderIntervalID);
		}
        );

		$('.game-img-slider').mouseout(function () {
		...
		$('.game-img-slider').on('swiperight', function () {
		    base.goBack();
		    return false;
		});
		...
		base.init = function(){
			...
			base.$left = base.$window.on('swipeleft', function () {
			    base.goBack();
			    return false;
			});
			//base.$right = base.$window.on('swiperight', function () {
			//    base.goForward();
			//    return false;
			//});
			sliderIntervalID = setInterval(function(){
		...
		base.setSizes = function(flag){
			...
			base.$el.css({
				...
				height   : Math.max.apply( this, base.heights ) + 20,
				// add padding so scrollLeft = 0 centers the left-most panel (needed because scrollLeft cannot be < 0)
				'padding-left' : (base.width - base.curWidth) / 2
			});
			base.$window.css({ height : (o.fixedHeight) ? Math.max.apply( this, base.heights ) + 20 : base.heights[base.curPanel - base.adj] + 20 });
		};
